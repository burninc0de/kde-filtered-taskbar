...
PlasmoidItem {
    ...
    property var toolTipOpenedByClick
    property var toolTipAreaItem
    ...
    Plasmoid.onUserConfiguringChanged: {
        if (Plasmoid.userConfiguring === true && groupDialog !== null) {
            groupDialog.visible = false;
        }
    }

    Layout.fillWidth: vertical ? true : false  // Don't fill horizontal panels to allow left-alignment
...
//END TODO
    ...
    Item {
        ...
        TriangleMouseFilter {
            id: tmf
            filterTimeOut: 300
            active: tasks.toolTipAreaItem ? tasks.toolTipAreaItem.toolTipOpen : false
            ...
            TaskList {
                ...
                Layout.maximumWidth: {
                    const totalMaxWidth = children.reduce((accumulator, child) => {
                            if (!isFinite(child.Layout.maximumWidth) || !child.visible) {
                                return accumulator;
                            }
                ...
                Layout.maximumHeight: {
                    const totalMaxHeight = children.reduce((accumulator, child) => {
                            if (!isFinite(child.Layout.maximumHeight) || !child.visible) {
                                return accumulator;
                            }
