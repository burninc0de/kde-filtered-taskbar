...
GridLayout {
    ...
    readonly property real minimumWidth: children.filter((item) => {
        return item.visible && item.width > 0;
    }).reduce((minimumWidth, item) => {
        return Math.min(minimumWidth, item.width);
    }, Infinity)
    readonly property int stripeCount: {
        ...
        const stripeSizeLimit = tasks.vertical ? Math.floor(tasks.width / children[0].implicitWidth) : Math.floor(tasks.height / children[0].implicitHeight);
        ...
        const maxTasksPerStripe = tasks.vertical ? Math.ceil(tasks.height / LayoutMetrics.preferredMinHeight()) : Math.ceil(tasks.width / LayoutMetrics.preferredMinWidth());
